...
def _format_output(data):
    ...
    """
    table_str = "--------------------------------------------------------------\n"
    table_str = table_str + "|first_name  |last_name   |birth       |email                |\n"
    for i in data:
        table_str = table_str + f"|{i.get('first_name')}       | {i.get('last_name')} | {i.get('birth')} | {i.get('email')} |\n|"
    table_str = table_str + "-------------------------------------------------------------"
    return table_str
...
def select_from_db(field, value):
    """
    Функция возвращает таблицу (строка) с релевантными результатами,
    где переданное значение встречается в переданном ключе.
    Форматирование результатов выполняет вспомогательная функция _format_output
    """
    list_str = list()
    for i in database:
        if i.get(field,None) == value:
            list_str.append(i)

    return _format_output(list_str)
...
table = select_from_db(field, value)
print(table)